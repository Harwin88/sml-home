/**
 * Script para crear las 32 FAQs iniciales
 * Ejecutar desde el contenedor Docker:
 * docker exec MSL-hogar-strapi npm run strapi -- exec ./src/scripts/seed-faqs-exec.js
 * 
 * O desde la terminal local (si Strapi está corriendo localmente):
 * npm run strapi -- exec ./src/scripts/seed-faqs-exec.js
 */
...
module.exports = async ({ strapi }) => {
    ...
    for (const faqData of faqsData) {
        try {
            // Usar entityService para crear y publicar en un solo paso
            const faq = await strapi.entityService.create('api::faq.faq', {
                data: {
                    ...faqData,
                    publishedAt: new Date() // ⭐ CRÍTICO: Publicar automáticamente
                }
            });
